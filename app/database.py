...
def optional_row_value(row: Any, key: str, default: Any = None) -> Any:
    try:
...
def is_postgres_budget_error(exc: BaseException) -> bool:
    """Recognize statement/lock budget expiry without matching error text."""
    if psycopg is None:
        return False
    return isinstance(exc, (psycopg.errors.QueryCanceled, psycopg.errors.LockNotAvailable))
...
def init_sqlite_db() -> None:
    with connect() as connection:
        ...
        ensure_column(connection, "users", "management_revision", "BIGINT NOT NULL DEFAULT 0")
        ...
        connection.execute(
            "CREATE INDEX IF NOT EXISTS idx_scan_jobs_dashboard_seek "
            "ON scan_jobs (source, id DESC) WHERE scan_role != 'child'"
        )
        connection.execute(
...
def init_postgres_db() -> None:
    with connect() as connection, postgres_schema_init_lock(connection):
        ...
        # int4 installations; do not take a table-rewrite lock every startup.
        size_type = connection.execute(
            "SELECT data_type FROM information_schema.columns "
            "WHERE table_schema = 'public' AND table_name = 'samples' "
            "AND column_name = 'size_bytes'"
        ).fetchone()
        if size_type is not None and row_value(size_type, "data_type") == "integer":
            connection.execute("ALTER TABLE samples ALTER COLUMN size_bytes TYPE BIGINT")
...
def ensure_service_client_schema(connection: Any) -> None:
    """Create the additive service-client/profile boundary in place.

    Existing scans remain nullable and therefore retain their historical
    manual/api/icap source semantics. New service submissions resolve a client
    and immutable profile snapshot before intake.
    """
    if using_postgres():
        connection.executescript(
            """
            ...
            CREATE TABLE IF NOT EXISTS scan_profiles (
                ...
                UNIQUE (service_client_id, name)
            );

            CREATE TABLE IF NOT EXISTS scan_profile_engines (
                scan_profile_id INTEGER NOT NULL,
                engine_instance_id INTEGER NOT NULL,
                required BOOLEAN NOT NULL DEFAULT TRUE,
                created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
                PRIMARY KEY (scan_profile_id, engine_instance_id),
            ...
            CREATE TABLE IF NOT EXISTS deferred_scan_submissions (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                service_client_id INTEGER NOT NULL,
                scan_profile_id INTEGER NOT NULL,
                ...
                UNIQUE (service_client_id, client_request_id)
            );
    ...
    ensure_column(connection, "scan_batches", "service_client_id", id_type)
    ensure_column(connection, "scan_batches", "scan_profile_id", id_type)
    ensure_column(connection, "scan_batches", "profile_snapshot_json", snapshot_type)
    connection.execute(
        "CREATE INDEX IF NOT EXISTS idx_scan_jobs_client_created "
        "ON scan_jobs (service_client_id, created_at, id)"
    )
    connection.execute(
        "CREATE INDEX IF NOT EXISTS idx_scan_batches_client_created "
        "ON scan_batches (service_client_id, created_at, id)"
    )
    connection.execute(
        "CREATE INDEX IF NOT EXISTS idx_api_client_credentials_client_active "
        "ON api_client_credentials (service_client_id, revoked_at, id)"
    )
    connection.execute(
        "CREATE INDEX IF NOT EXISTS idx_scan_jobs_ledger_seek ON scan_jobs (id DESC) "
        "WHERE source IN ('api', 'icap') AND scan_role != 'child'"
    )
    connection.execute(
        "CREATE INDEX IF NOT EXISTS idx_scan_jobs_ledger_client_seek ON scan_jobs (service_client_id, id DESC) "
        "WHERE source IN ('api', 'icap') AND scan_role != 'child'"
    )
    connection.execute(
        "CREATE INDEX IF NOT EXISTS idx_api_client_credentials_client_seek "
        "ON api_client_credentials (service_client_id, id)"
    )
    connection.execute(
        ...
        with connect() as connection:
            if password_hash is None:
                connection.execute(
                    """
                    UPDATE users
                    SET role = ?, management_revision = management_revision + 1, updated_at = CURRENT_TIMESTAMP
                    WHERE id = ?
                    """,
                    (role, user_id),
                )
                ...
                connection.execute(
                    """
                    UPDATE users
                    SET role = ?, password_hash = ?, management_revision = management_revision + 1, updated_at = CURRENT_TIMESTAMP
                    WHERE id = ?
                    """,
                    (role, password_hash, user_id),
                )
                # Password replacement and revocation must commit together.
    ...
    with connect() as connection:
        ...
        if row is None:
            ...
            connection.execute(
                """
                UPDATE users
                SET role = ?, external_id = ?, display_name = ?, management_revision = management_revision + 1,
...
def count_users_by_role(role: str, auth_source: str | None = None) -> int:
    try:
        with connect() as connection:
            if auth_source is None:
                ...
                row = connection.execute(
                    "SELECT COUNT(*) AS user_count FROM users WHERE role = ? AND auth_source = ?",
                    (role, auth_source),
                ).fetchone()
    except sqlite3.OperationalError as exc:
        if not is_missing_users_table(exc):
    ...
    if row is None:
        return 0
    return int(row_value(row, 'user_count'))
...
def change_local_user_password(user_id: int, expected_hash: str, password_hash: str,
                               *, lock_timeout_ms: int | None = None) -> bool:
    """Replace only the verified password, preserving concurrently changed roles.

    The UPDATE locks the user row until session revocation commits. Local login
    locks that same row before checking its verified hash and creating a session.
    """
    with connect() as connection:
        if using_postgres() and lock_timeout_ms is not None:
            connection.execute("SELECT set_config('lock_timeout', ?, true)",
                               (f'{max(100, min(lock_timeout_ms, 60000))}ms',))
        cursor = connection.execute('''UPDATE users
            SET password_hash = ?, management_revision = management_revision + 1, updated_at = CURRENT_TIMESTAMP
            WHERE id = ? AND auth_source = 'local' AND password_hash = ?''',
...
def create_auth_session(user_id: int, token_hash: str, expires_at: int,
                        *, expected_password_hash: str | None = None) -> int | None:
    try:
        with connect() as connection:
            if expected_password_hash is not None:
                ...
                row = connection.execute(
                    'SELECT password_hash, auth_source FROM users WHERE id = ?' + lock,
                    (user_id,),
                ).fetchone()
                if row is None or row['auth_source'] != 'local' or row['password_hash'] != expected_password_hash:
                    return None
            cursor = connection.execute(
                f"""
    ...
    *,
    ...
    with connect() as connection:
        cursor = connection.execute(
            ...
            {returning_id_clause()}
            """,
            (client_key, display_name, db_bool(enabled)),
        )
        return require_lastrowid(cursor)


def create_service_client_bundle(
    *,
    client_key: str,
    display_name: str,
    profile_name: str,
    ...
    with connect() as connection:
        ...
        engine_rows = connection.execute(
            ...
            tuple(unique_engine_ids),
        ).fetchall()
        if len(engine_rows) != len(unique_engine_ids):
        ...
        client_cursor = connection.execute(
            ...
            {returning_id_clause()}
            """,
            (client_key, display_name, db_bool(True)),
        )
        client_id = require_lastrowid(client_cursor)
        ...
        credential_cursor = connection.execute(
            f"""
            INSERT INTO api_client_credentials (
                service_client_id, label, token_hash, token_prefix
            ) VALUES (?, ?, ?, ?)
            {returning_id_clause()}
            """,
            (client_id, credential_label, token_hash, token_prefix),
        )
        return client_id, profile_id, require_lastrowid(credential_cursor)


def get_service_client(client_id: int) -> ServiceClientRecord | None:
    with connect() as connection:
        row = connection.execute(
            "SELECT * FROM service_clients WHERE id = ?", (client_id,)
        ).fetchone()
    return None if row is None else row_to_service_client_record(row)


def get_service_client_by_key(client_key: str) -> ServiceClientRecord | None:
    with connect() as connection:
        row = connection.execute(
            "SELECT * FROM service_clients WHERE client_key = ?", (client_key,)
        ).fetchone()
    return None if row is None else row_to_service_client_record(row)


def list_service_clients() -> list[ServiceClientRecord]:
    with connect() as connection:
        rows = connection.execute(
            "SELECT * FROM service_clients ORDER BY display_name ASC, id ASC"
        ).fetchall()
    return [row_to_service_client_record(row) for row in rows]
...
HASH_LIST_KINDS = ("block", "allow")


def ensure_hash_list_schema(connection: Any) -> None:
    """One institution-wide SHA-256 list; a hash is either blocked or allowed.

    The unique digest makes the two lists mutually exclusive, so a hash can
    never be both, and moving one between lists is an explicit edit rather
    than a second row that silently wins or loses.
    """
    id_column = ("id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
                 if using_postgres() else "id INTEGER PRIMARY KEY AUTOINCREMENT")
    connection.execute(f'''CREATE TABLE IF NOT EXISTS hash_list_entries (
        {id_column},
        sha256 TEXT NOT NULL UNIQUE,
        list_kind TEXT NOT NULL CHECK (list_kind IN ('block', 'allow')),
        note TEXT NOT NULL DEFAULT '',
        created_by TEXT,
        created_at BIGINT NOT NULL
    )''')
    connection.execute('''CREATE INDEX IF NOT EXISTS idx_hash_list_entries_kind_seek
        ON hash_list_entries (list_kind, id DESC)''')


def get_hash_list_entry(sha256: str) -> dict | None:
    with connect() as connection:
        row = connection.execute("""SELECT id, sha256, list_kind, note, created_at
            FROM hash_list_entries WHERE sha256 = ?""", (sha256,)).fetchone()
...
def hash_list_counts() -> dict[str, int]:
    ...
    counts = {kind: 0 for kind in HASH_LIST_KINDS}
    counts.update({str(row["list_kind"]): int(row["entries"]) for row in rows})
    return counts


def add_hash_list_entries(entries: list[tuple[str, str, str]],
                          created_by: str | None) -> list[tuple[str, str]]:
    """Insert (sha256, list_kind, note) rows in one transaction.

    An existing digest is left untouched, whichever list it is on, and
    returned with the list it is actually on: silently re-classifying a
    blocked hash as allowed is exactly the edit that must never happen by
    accident.
    """
    now = int(time.time())
    existing: list[tuple[str, str]] = []
    with connect() as connection:
        for sha256, list_kind, note in entries:
            cursor = connection.execute("""INSERT INTO hash_list_entries
                (sha256, list_kind, note, created_by, created_at) VALUES (?, ?, ?, ?, ?)
                ON CONFLICT (sha256) DO NOTHING""", (sha256, list_kind, note, created_by, now))
            if not cursor.rowcount:
                row = connection.execute("SELECT list_kind FROM hash_list_entries WHERE sha256 = ?",
                                         (sha256,)).fetchone()
                # A concurrent removal can leave nothing to report; the hash was
                # still not added by this request.
                existing.append((sha256, str(row["list_kind"]) if row is not None else "removed"))
    return existing


def delete_hash_list_entry(entry_id: int) -> bool:
    with connect() as connection:
        cursor = connection.execute("DELETE FROM hash_list_entries WHERE id = ?", (entry_id,))
        return bool(cursor.rowcount)
...
def create_scan_profile(
    ...
    *,
    ...
    with profile_write_transaction(service_client_id, lock_timeout_ms) as (connection, client):
        ...
        profile_id = require_lastrowid(cursor)
        for engine_id in unique_engine_ids:
            connection.execute(
                """
                INSERT INTO scan_profile_engines (
...
def get_scan_profile(profile_id: int) -> ScanProfileRecord | None:
    with connect() as connection:
        row = connection.execute(
            "SELECT * FROM scan_profiles WHERE id = ?", (profile_id,)
        ).fetchone()
    return None if row is None else row_to_scan_profile_record(row)


def get_default_scan_profile_for_client(
    service_client_id: int,
) -> ScanProfileRecord | None:
    with connect() as connection:
        row = connection.execute(
            """
            ...
            LIMIT 1
            """,
            (service_client_id, db_bool(True)),
        ).fetchone()
    return None if row is None else row_to_scan_profile_record(row)
...
def list_scan_profiles(
    ...
    with connect() as connection:
        rows = connection.execute(
            ...
            SELECT * FROM scan_profiles
            {where_sql}
            ORDER BY service_client_id ASC, is_default DESC, name ASC, id ASC
            """,
            params,
        ).fetchall()
    return [row_to_scan_profile_record(row) for row in rows]


def set_scan_profile_engines(
    profile_id: int,
    engine_instance_ids: list[int],
    ...
    with profile_write_transaction(owner['service_client_id'], lock_timeout_ms) as (connection, _client):
        ...
        if expected_engine_ids is not None:
            ...
            if [row['engine_instance_id'] for row in current] != sorted(set(expected_engine_ids)):
                raise ValueError('Profile routing changed. Refresh before saving.')
        placeholders = ", ".join("?" for _ in unique_engine_ids)
        rows = connection.execute(
            f"SELECT id FROM engine_instances WHERE id IN ({placeholders})",
            tuple(unique_engine_ids),
        ).fetchall()
        if len(rows) != len(unique_engine_ids):
            raise ValueError("One or more engine instances do not exist.")
        connection.execute(
            "DELETE FROM scan_profile_engines WHERE scan_profile_id = ?", (profile_id,)
        )
        for engine_id in unique_engine_ids:
            connection.execute(
                """
...
def list_scan_profile_engines(profile_id: int) -> list[EngineInstanceRecord]:
    with connect() as connection:
        rows = connection.execute(
            """
            SELECT engine_instances.*
            FROM scan_profile_engines
            JOIN engine_instances
              ON engine_instances.id = scan_profile_engines.engine_instance_id
            WHERE scan_profile_engines.scan_profile_id = ?
            ORDER BY engine_instances.display_name ASC, engine_instances.id ASC
            """,
            (profile_id,),
        ).fetchall()
...
def create_api_client_credential(
    service_client_id: int,
    *,
    label: str,
    token_hash: str,
...
def get_api_client_credential_by_hash(
    token_hash: str,
    *,
    current_time: int,
) -> ApiClientCredentialRecord | None:
    with connect() as connection:
        row = connection.execute(
            """
            SELECT * FROM api_client_credentials
        ...
        if row is None:
            return None
        credential_id = int(row_value(row, "id"))
        connection.execute(
            "UPDATE api_client_credentials SET last_used_at = ? WHERE id = ?",
            (current_time, credential_id),
        )
    return row_to_api_client_credential_record(row)


def list_api_client_credentials(
    service_client_id: int,
) -> list[ApiClientCredentialRecord]:
    with connect() as connection:
        rows = connection.execute(
            """
            SELECT * FROM api_client_credentials
    ...
    return [row_to_api_client_credential_record(row) for row in rows]


def has_active_api_client_credentials() -> bool:
    with connect() as connection:
        row = connection.execute(
            """
...
def revoke_api_client_credential(credential_id: int, current_time: int) -> bool:
    with connect() as connection:
        cursor = connection.execute(
            """
...
def create_deferred_scan_submission(
    *,
    service_client_id: int,
    scan_profile_id: int,
...
) -> tuple[DeferredScanRecord, bool]:
    """Idempotently accept one deferred object reference per client request id."""
    with connect() as connection:
        cursor = connection.execute(
            f"""
        ...
        if using_postgres():
            ...
            inserted_id = (
                None if inserted_row is None else int(row_value(inserted_row, "id"))
            )
        else:
            inserted_id = (
                int(cursor.lastrowid)
                if int(cursor.rowcount) > 0 and cursor.lastrowid is not None
                else None
            )
        row = connection.execute(
            """
            SELECT * FROM deferred_scan_submissions
            WHERE service_client_id = ? AND client_request_id = ?
            """,
...
def get_deferred_scan_submission(submission_id: int) -> DeferredScanRecord | None:
    with connect() as connection:
        row = connection.execute(
            "SELECT * FROM deferred_scan_submissions WHERE id = ?", (submission_id,)
        ).fetchone()
    return None if row is None else row_to_deferred_scan_record(row)


def claim_next_deferred_scan_submission(
    backend_keys: set[str],
    worker_id: str,
    *,
    lease_seconds: int,
    now: int | None = None,
) -> DeferredScanRecord | None:
    if not backend_keys:
        return None
    current_time = int(time.time()) if now is None else now
    keys = sorted(backend_keys)
    ...
    with connect() as connection:
        ...
        row = connection.execute(
            f"""
            SELECT id FROM deferred_scan_submissions
            WHERE status = 'pending'
              AND available_at <= ?
              AND backend_key IN ({placeholders})
        ...
        if row is None:
            return None
        submission_id = int(row_value(row, "id"))
        connection.execute(
            """
            UPDATE deferred_scan_submissions
            SET status = 'claimed', worker_id = ?, lease_expires_at = ?,
                attempt_count = attempt_count + 1, last_error = NULL,
                updated_at = CURRENT_TIMESTAMP
            WHERE id = ? AND status = 'pending'
            """,
            (worker_id, current_time + max(30, lease_seconds), submission_id),
        )
        claimed = connection.execute(
            "SELECT * FROM deferred_scan_submissions WHERE id = ?", (submission_id,)
        ).fetchone()
    return None if claimed is None else row_to_deferred_scan_record(claimed)


def renew_deferred_scan_lease(
    submission_id: int,
    worker_id: str,
    generation: int,
    lease_seconds: int,
    *,
    ...
    with connect() as connection:
        cursor = connection.execute(
            ...
            """,
            (
                current_time + max(30, lease_seconds),
                submission_id,
                worker_id,
                generation,
            ),
...
def complete_deferred_scan_intake(
    *,
    submission_id: int,
    worker_id: str,
    generation: int,
    ...
    with connect() as connection:
        ...
        row = connection.execute(
            ...
            """,
            (submission_id, worker_id, generation),
        ).fetchone()
        if row is None:
            return None
                ...
                metadata_json=json.dumps(
                    {
                        ...
                        "deferred_submission_id": request.id,
                    }
                ),
        ...
        cursor = connection.execute(
            """
            UPDATE deferred_scan_submissions
            SET status = 'queued', scan_job_id = ?, worker_id = NULL,
                lease_expires_at = NULL, last_error = NULL,
                updated_at = CURRENT_TIMESTAMP
            WHERE id = ? AND status = 'claimed' AND worker_id = ?
              AND attempt_count = ?
            """,
            (scan_id, submission_id, worker_id, generation),
        )
    ...
    with connect() as connection:
        cursor = connection.execute(
            ...
            SET status = ?, worker_id = NULL, lease_expires_at = NULL,
                available_at = ?, last_error = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ? AND status = 'claimed' AND worker_id = ?
              AND attempt_count = ?
            """,
            (
...
def recover_expired_deferred_scan_intakes(*, now: int | None = None) -> int:
    current_time = int(time.time()) if now is None else now
    with connect() as connection:
        cursor = connection.execute(
            """
            ...
            SET status = 'pending', worker_id = NULL, lease_expires_at = NULL,
                available_at = ?, last_error = 'Recovered after intake worker lease expired.',
                updated_at = CURRENT_TIMESTAMP
...
def ensure_legacy_service_client_profile(
    engine_instance_ids: list[int],
    ...
    with connect() as connection:
        ...
        connection.execute(
            """
            INSERT INTO service_clients (client_key, display_name, enabled, updated_at)
            VALUES ('legacy-default', 'Legacy API / ICAP', ?, CURRENT_TIMESTAMP)
        ...
        for engine_id in sorted(set(engine_instance_ids)):
            cursor = connection.execute(
                ...
                ON CONFLICT(scan_profile_id, engine_instance_id) DO NOTHING
                """,
                (profile_id, engine_id, db_bool(True)),
            )
...
def list_engine_instances_by_ids(
    engine_instance_ids: list[int],
) -> list[EngineInstanceRecord]:
    unique_ids = sorted(set(engine_instance_ids))
    if not unique_ids:
        return []
...
def update_engine_instance_by_id(
    ...
    # Update only submitted columns. A concurrent enable/config operation must
    # not write back unrelated values read before the other transaction.
    changes: dict[str, object] = {}
    if display_name is not None:
        changes["display_name"] = display_name
    if enabled is not None:
        changes["enabled"] = db_bool(enabled)
    ...
    with connect() as connection:
        cursor = connection.execute(
            "UPDATE engine_instances SET " + ", ".join(f"{key} = ?" for key in changes)
            + ", updated_at = CURRENT_TIMESTAMP WHERE id = ?",
            (*changes.values(), instance_id),
        )
        updated = int(cursor.rowcount or 0) > 0
        if updated and config_json is not None:
            # A successful probe of the old configuration must not be visible
            # as current after the new configuration commits.
            connection.execute(
                "UPDATE engine_node_health SET last_checked_at = NULL, check_worker_id = NULL, "
                "check_lease_expires_at = NULL, updated_at = CURRENT_TIMESTAMP WHERE engine_instance_id = ?",
                (instance_id,),
            )
    return updated
    ...
    with connect() as connection:
        rows = connection.execute(
            ...
            SELECT
                ...
                scan_jobs.scan_role,
                scan_jobs.service_client_id,
                scan_jobs.scan_profile_id,
                scan_jobs.profile_snapshot_json,
                scan_jobs.status,
                scan_jobs.verdict,
...
def build_scan_history_where_clause(
    *,
    ...
    if service_client_id is not None:
        conditions.append("scan_jobs.service_client_id = ?")
        params.append(service_client_id)
...
def count_scans_older_than(created_before: str) -> int:
    with connect() as connection:
        row = connection.execute(
            """SELECT COUNT(*) AS total FROM scan_jobs
               WHERE COALESCE(completed_at, failed_at, created_at) < ?
                 AND status IN ('completed', 'failed')
                 AND NOT EXISTS (
                    SELECT 1 FROM notification_outbox
                    WHERE notification_outbox.scan_job_id = scan_jobs.id
                      AND notification_outbox.status != 'delivered'
                 )""",
            (created_before,),
        ).fetchone()
    if row is None:
    ...
    with connect() as connection:
        rows = connection.execute(
            ...
            WHERE COALESCE(
              ...
              AND scan_jobs.status IN ('completed', 'failed')
              AND NOT EXISTS (
                    SELECT 1 FROM notification_outbox
                    WHERE notification_outbox.scan_job_id = scan_jobs.id
                      AND notification_outbox.status != 'delivered'
...
def list_active_scans(limit: int = 20, offset: int = 0) -> list[ScanRecord]:
    with connect() as connection:
        rows = connection.execute(
            """
            SELECT
...
def delete_scan(scan_id: int, *, expected_attempt: int | None = None,
                ...
                lock_timeout_ms: int | None = None,
                created_before: str | None = None) -> ScanRecord | None:
    ...
    with connect() as connection:
        if using_postgres() and lock_timeout_ms is not None:
            connection.execute("SELECT set_config('lock_timeout', ?, true)",
                               (f'{max(100, min(int(lock_timeout_ms), 60000))}ms',))
        if not using_postgres():
            connection.execute("BEGIN IMMEDIATE")
        row = connection.execute(
            "SELECT status, attempt_count, source, scan_role FROM scan_jobs WHERE id = ? "
            + ("AND created_at < ? " if created_before is not None else "")
            + ("FOR UPDATE" if using_postgres() else ""),
            (scan_id, created_before) if created_before is not None else (scan_id,),
        ).fetchone()
        ...
        if expected_job_revision is not None and connection.execute(
                'SELECT COALESCE(MAX(id), 0) AS revision FROM scan_engine_jobs WHERE scan_job_id = ?',
                (scan_id,)).fetchone()['revision'] != expected_job_revision:
            return None
        # A sample deletion cascades to every referencing scan. Do not delete
        # another run through a shared sample, or detach archive navigation in
        # the browser's single-record operation.
        if using_postgres():
            # Serialize against a concurrent scan attaching to this sample via
            # its foreign key between the reference check and cascade delete.
            connection.execute('SELECT id FROM samples WHERE id = ? FOR UPDATE', (scan.sample_id,)).fetchone()
        if connection.execute("SELECT 1 FROM scan_jobs WHERE sample_id = ? AND id != ? LIMIT 1",
                              (scan.sample_id, scan_id)).fetchone() is not None:
            return None
        if protect_children and connection.execute(
                "SELECT 1 FROM scan_jobs WHERE parent_scan_id = ? LIMIT 1", (scan_id,)).fetchone() is not None:
            return None
        pending_event = connection.execute(
            """
            SELECT 1 FROM notification_outbox
            WHERE scan_job_id = ? AND status != 'delivered'
            LIMIT 1
            """,
            (scan_id,),
        ).fetchone()
...
def _settle_deferred_submission(connection: Any, scan_id: int, status: str) -> None:
    connection.execute(
        ...
        WHERE scan_job_id = ? AND status IN ('queued', 'scanning')
        """,
        (status, scan_id),
    )
...
def _enqueue_scan_notification_if_requested(
    ...
    for result in connection.execute(
        """
        SELECT engine_name, signature, severity, confidence
        FROM engine_results
        WHERE scan_job_id = ? AND status = 'completed' AND detected = ?
        ORDER BY id ASC
        """,
        (scan_id, db_bool(True)),
    ).fetchall():
        detections.append(
            {
                "engine": str(row_value(result, "engine_name")),
                "signature": row_value(result, "signature"),
                "severity": str(row_value(result, "severity")),
                "confidence": int(row_value(result, "confidence")),
            }
        )
    ...
    payload = {
        ...
        "idempotency_key": f"scan:{scan_id}:malware.detected",
        "scan_id": scan_id,
    ...
    connection.execute(
        ...
        (
            scan_id,
            int(row_value(scan, "service_client_id")),
...
def claim_next_notification_outbox(
    worker_id: str,
    *,
    lease_seconds: int,
    now: int | None = None,
    ...
    with connect() as connection:
        ...
        if row is None:
            return None
        event_id = int(row_value(row, "id"))
        connection.execute(
            """
            UPDATE notification_outbox
            SET status = 'delivering', worker_id = ?, lease_expires_at = ?,
                attempt_count = attempt_count + 1, updated_at = CURRENT_TIMESTAMP
            WHERE id = ? AND status = 'pending'
            """,
            (worker_id, current_time + max(30, lease_seconds), event_id),
        )
        claimed = connection.execute(
            "SELECT * FROM notification_outbox WHERE id = ?", (event_id,)
        ).fetchone()
    return None if claimed is None else row_to_notification_outbox_record(claimed)


def mark_notification_delivered(
    event_id: int, worker_id: str, generation: int
) -> bool:
    with connect() as connection:
        cursor = connection.execute(
            """
            ...
            SET status = 'delivered', worker_id = NULL, lease_expires_at = NULL,
                ...
                updated_at = CURRENT_TIMESTAMP
            WHERE id = ? AND status = 'delivering' AND worker_id = ?
              AND attempt_count = ?
            """,
    ...
    with connect() as connection:
        cursor = connection.execute(
            ...
            WHERE id = ? AND status = 'delivering' AND worker_id = ?
              AND attempt_count = ?
            """,
            (retry_at, error[:2000], event_id, worker_id, generation),
        )
        return int(cursor.rowcount) > 0


def recover_expired_notification_deliveries(*, now: int | None = None) -> int:
    current_time = int(time.time()) if now is None else now
    with connect() as connection:
        cursor = connection.execute(
            """
            UPDATE notification_outbox
            SET status = 'pending', worker_id = NULL, lease_expires_at = NULL,
                available_at = ?, last_error = 'Recovered after notification lease expired.',
                updated_at = CURRENT_TIMESTAMP
            WHERE status = 'delivering' AND lease_expires_at IS NOT NULL
              AND lease_expires_at <= ?
            """,
            (current_time, current_time),
        )
...
def get_scan_counts(
    ...
    *,
    ...
    with connect() as connection:
        ...
        if service_client_id is not None:
            conditions.append("service_client_id = ?")
            params.append(service_client_id)
...
def get_queue_metrics(*, connection=None) -> dict[str, int]:
...
def get_scan_queue_position(scan_id: int, *, connection=None) -> int | None:
    with (connect() if connection is None else nullcontext(connection)) as connection:
        row = connection.execute(
            """
...
def retry_scan_job(scan_id: int, *, expected_attempt: int | None = None,
...
def row_to_scan_record(row: sqlite3.Row) -> ScanRecord:
    return ScanRecord(
        ...
        if optional_row_value(row, "service_client_id") is None
        else int(optional_row_value(row, "service_client_id")),
        scan_profile_id=None
        if optional_row_value(row, "scan_profile_id") is None
        else int(optional_row_value(row, "scan_profile_id")),
        profile_snapshot_json=str(
            optional_row_value(row, "profile_snapshot_json", "{}") or "{}"
        ),
    )
...
def row_to_service_client_record(row: Any) -> ServiceClientRecord:
    return ServiceClientRecord(
        id=int(row_value(row, "id")),
        client_key=str(row_value(row, "client_key")),
        display_name=str(row_value(row, "display_name")),
        enabled=bool(int(row_value(row, "enabled"))),
        created_at=str(row_value(row, "created_at")),
        updated_at=str(row_value(row, "updated_at")),
...
def row_to_scan_profile_record(row: Any) -> ScanProfileRecord:
    return ScanProfileRecord(
        id=int(row_value(row, "id")),
        service_client_id=int(row_value(row, "service_client_id")),
        name=str(row_value(row, "name")),
        enabled=bool(int(row_value(row, "enabled"))),
        is_default=bool(int(row_value(row, "is_default"))),
        policy_json=str(row_value(row, "policy_json") or "{}"),
        created_at=str(row_value(row, "created_at")),
        updated_at=str(row_value(row, "updated_at")),
    )


def row_to_api_client_credential_record(row: Any) -> ApiClientCredentialRecord:
    return ApiClientCredentialRecord(
        id=int(row_value(row, "id")),
        service_client_id=int(row_value(row, "service_client_id")),
        label=str(row_value(row, "label")),
...
def row_to_deferred_scan_record(row: Any) -> DeferredScanRecord:
    return DeferredScanRecord(
        id=int(row_value(row, "id")),
        service_client_id=int(row_value(row, "service_client_id")),
        ...
        expected_size_bytes=(
            None
            if row_value(row, "expected_size_bytes") is None
            else int(row_value(row, "expected_size_bytes"))
        ),
        expected_sha256=(
            None
            if row_value(row, "expected_sha256") is None
            else str(row_value(row, "expected_sha256"))
        ),
        archive_mode=str(row_value(row, "archive_mode")),
        case_name=str(row_value(row, "case_name")),
        priority=str(row_value(row, "priority")),
        note=str(row_value(row, "note")),
        profile_snapshot_json=str(row_value(row, "profile_snapshot_json")),
        status=str(row_value(row, "status")),
        scan_job_id=(
            None
            if row_value(row, "scan_job_id") is None
            else int(row_value(row, "scan_job_id"))
        ),
        worker_id=(
            None if row_value(row, "worker_id") is None else str(row_value(row, "worker_id"))
        ),
        lease_expires_at=(
            None
            if row_value(row, "lease_expires_at") is None
        ...
        attempt_count=int(row_value(row, "attempt_count")),
        available_at=int(row_value(row, "available_at")),
        last_error=(
            None if row_value(row, "last_error") is None else str(row_value(row, "last_error"))
        ),
        created_at=str(row_value(row, "created_at")),
        updated_at=str(row_value(row, "updated_at")),
    )
...
def row_to_notification_outbox_record(row: Any) -> NotificationOutboxRecord:
    return NotificationOutboxRecord(
        id=int(row_value(row, "id")),
        scan_job_id=int(row_value(row, "scan_job_id")),
        service_client_id=int(row_value(row, "service_client_id")),
        event_type=str(row_value(row, "event_type")),
        idempotency_key=str(row_value(row, "idempotency_key")),
        payload_json=str(row_value(row, "payload_json")),
        status=str(row_value(row, "status")),
        worker_id=(
            None if row_value(row, "worker_id") is None else str(row_value(row, "worker_id"))
        ),
        ...
        delivered_at=(
            None
            if row_value(row, "delivered_at") is None
            else str(row_value(row, "delivered_at"))
        ),
        created_at=str(row_value(row, "created_at")),
        updated_at=str(row_value(row, "updated_at")),
    )
