...
| ESET Server Security for Linux via local CLI (odscan) | ESET | ESET Server Security for Linux | local CLI (odscan) | research | Adapter skeleton implemented (disabled on add, Linux-only, `eset_server_security_linux_cli`). Stage B needs: Phase 0 license confirmation, dedicated ESET PROTECT group/policy with a staging-only real-time exclusion, real odscan exit-code/output fixtures, and confirmation that `--ignore-exclusions`/version flags are supported. See `ESET_SERVER_SECURITY.md`. |
| ESET Server Security via ICAP | ESET | ESET Server Security or ICAP-capable gateway product | ICAP | research | Local CLI (odscan) adapter path selected instead; this ICAP alternative is on hold. Would need exact product, ICAP service behavior, clean/detected responses, headers, licensing, and file size limits. |
| ESET PROTECT management | ESET | ESET PROTECT | management plane | not_supported | Not a scan engine. Used for policy, licensing, updates, and telemetry; MASP does not submit files to PROTECT. Scanning is performed by the local `odscan` adapter. |
