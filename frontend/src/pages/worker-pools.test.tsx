...
describe('Worker pools', () => {
  it('confirms creation with the exact typed name and selector and CSRF', async () => {
    const fetcher = mount()
    expect(await screen.findByRole('button', { name: 'Manage pool 7' })).toHaveTextContent('<script>Pool</script>')
    expect(document.querySelector('script')).toBeNull()
    await userEvent.click(screen.getByRole('button', { name: 'New pool' }))
  ...
  it('submits an explicit disabled state for the selected pool only', async () => {
    const fetcher = mount()
    await userEvent.click(await screen.findByRole('button', { name: 'Manage pool 7' }))
    const form = within(screen.getByRole('form', { name: 'Edit pool 7' }))
  ...
  it('cancels deletion without writes and never retries an uncertain failure', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Manage pool 7' }))
    await userEvent.click(screen.getByRole('button', { name: 'Delete pool' }))
