...
export default function ArchiveChildren() {
  ...
  const children = useQuery({ queryKey: ['archive-children', scanId, query], enabled: valid,
    queryFn: ({ signal }) => request('/api/ui/v1/scans/{scan_id}/children', 'get', { params: { scan_id: Number(scanId) }, query: new URLSearchParams(query), signal }), retry: false,
  ...
  return <section className="page"><div className="page-heading"><div><p className="eyebrow">REGISTERED CHILD SCANS</p>
    ...
    <div className="report-actions"><Link className="button button-secondary" to={`/scans/${scanId}`}>Parent report</Link>
    ...
    {children.error ? <section className="error" role="alert"><p>{children.error.message}</p>
      ...
      {page.items.length === 0 ? <section className="empty"><h2>No registered children on this page</h2>
        ...
        <div className="history-table-wrap" tabIndex={0} role="region" aria-label="Archive child scans"><table className="history-table">
          ...
          <tbody>{page.items.map(child => <tr key={child.id}><td><Link className="sample-link" to={`/scans/${child.id}`}>{child.path}{child.path_truncated ? '…' : ''}</Link>
            ...
            <td>{child.has_children ? <Link to={`/scans/${child.id}/children`}>Browse children of #{child.id}</Link> : <Link to={`/scans/${child.id}`}>Open report #{child.id}</Link>}</td>
          </tr>)}</tbody></table></div>}
      ...
      <p className="callout">{page.parent_scan_id && <><Link to={`/scans/${page.parent_scan_id}/children`}>Up one level</Link> · </>}
        {page.batch_id && <><Link to={`/batches/${page.batch_id}`}>Batch overview</Link> · </>}
        <a href={`/scans/${scanId}`}>Legacy report: full output</a></p>
    </>}
  </section>
}
