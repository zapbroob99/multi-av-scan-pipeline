...
export default function BatchOverview() {
  ...
  const batch = useQuery({ queryKey: ['batch-overview', batchId, query.toString()], enabled: valid,
    queryFn: ({ signal }) => request('/api/ui/v1/batches/{batch_id}', 'get', {
  ...
  if (!valid) return <section className="page"><h1>Invalid batch ID</h1><Link to="/dashboard">Dashboard</Link></section>
  return <section className="page"><div className="page-heading"><div><p className="eyebrow">MANUAL ARCHIVE BATCH #{batchId}</p>
    <h1>Batch overview</h1><p className="muted report-filename">{page?.filename || `Batch #${batchId}`}{page?.filename_truncated ? '…' : ''}</p></div>
    <div className="report-actions"><Link className="button button-secondary" to="/dashboard">Dashboard</Link>
      <Button variant="secondary" disabled={batch.isFetching} onClick={() => { void batch.refetch() }}>Refresh batch</Button></div></div>
    <p className="callout">This page lists registered manual scans. Recorded counts and risk may lag active workers and do not prove clean coverage or complete extraction. Open each report for the backend policy decision and required-engine coverage.</p>
    ...
    {batch.error ? <section className="error" role="alert"><p>{batch.error.message}</p>
      ...
      {page.items.length === 0 ? <section className="empty"><h2>No registered scans on this page</h2>
        ...
        <div className="history-table-wrap" tabIndex={0} role="region" aria-label="Batch scans"><table className="history-table">
          ...
          <tbody>{page.items.map(scan => <tr key={scan.id}><td><Link className="sample-link" to={`/scans/${scan.id}`}>{scan.path}{scan.path_truncated ? '…' : ''}</Link>
