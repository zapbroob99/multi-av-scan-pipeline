...
test('analyst browses API/ICAP history, filters ownership and uses seek pagination', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'ledger-00.bin' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'ledger-20.bin' })).toHaveCount(0)
  ...
  await page.getByRole('button', { name: 'Apply filters' }).click()
  await expect(page.getByRole('heading', { name: 'ledger-20.bin' })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'ledger-21.bin' })).toHaveCount(0)
